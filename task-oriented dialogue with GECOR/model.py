...
from dialogue_with_GECOR import GECOR, cuda_
...
class Model:
    ...
    def train(self):
        ...
        for epoch in range(cfg.epoch_num):
            ...
            for iter_num, dial_batch in enumerate(data_iterator):
                ...
                for turn_num, turn_batch in enumerate(dial_batch):
                    ...
                    optim.zero_grad()
                    history_dialogue_np, history_dialogue, history_dial_len, u_complete_input, u_complete_input_np, u_complete_len, u_input, u_input_np, z_input, \
                    m_input, m_input_np, u_len, m_len, degree_input, kw_ret \
                        = self._convert_batch(turn_batch, prev_z)

                    loss, pr_loss, m_loss, c_loss, turn_states = self.m(history_dialogue_np=history_dialogue_np,
                                                                        history_dialogue=history_dialogue,
                                                                        history_dial_len=history_dial_len,
                                                                        u_complete_input=u_complete_input,
                                                                        u_complete_input_np=u_complete_input_np,
                                                                        u_complete_len=u_complete_len,
                                                                        u_input=u_input, z_input=z_input,
                                                                        m_input=m_input,
                    ...
                    sup_cnt += 1
                    # logging.debug(
                    #     'loss:{} pr_loss:{} m_loss:{} c_loss:{} grad:{}'.format(loss.data[0], pr_loss.data[0],
                    #                                                             m_loss.data[0], c_loss.data[0], grad))
            ...
            valid_sup_loss, valid_unsup_loss, eval_res, resolution_bleu_score, user_f1, user_accuracy = self.validate()
            ...
            if eval_res >= prev_max_success:
                self.save_model(epoch)
                print('model has been saved.')
                prev_max_success = eval_res
        ...
        print('max_success_f1:', prev_max_success)
        ...
        for batch_num, dial_batch in enumerate(data_iterator):
            ...
            for turn_num, turn_batch in enumerate(dial_batch):
                ...
                m_len, degree_input, kw_ret \
                    = self._convert_batch(turn_batch, prev_z)
                m_idx, z_idx, turn_states, c_idx = self.m(history_dialogue_np=history_dialogue_np,
                                                          history_dialogue=history_dialogue,
                                                          history_dial_len=history_dial_len,
                                                          u_complete_input=u_complete_input,
                                                          u_complete_input_np=u_complete_input_np,
                                                          u_complete_len=u_complete_len, u_input=u_input,
                                                          z_input=z_input, m_input=m_input, degree_input=degree_input,
                                                          u_input_np=u_input_np, m_input_np=m_input_np,
                                                          turn_states=turn_states, u_len=u_len, m_len=m_len,
                                                          mode='test', **kw_ret)
                self.reader.wrap_result(turn_batch, c_idx, m_idx, z_idx, prev_z=prev_z)
                prev_z = z_idx
        ev = self.EV(result_path=cfg.result_path)
        res, resolution_bleu_score, user_f1, user_accuracy = ev.run_metrics()
        self.m.train()
        return res, resolution_bleu_score, user_f1, user_accuracy
        ...
        for dial_batch in data_iterator:
            ...
            for turn_num, turn_batch in enumerate(dial_batch):
                ...
                loss, pr_loss, m_loss, c_loss, turn_states = self.m(history_dialogue_np=history_dialogue_np,
                                                                    ...
                                                                    u_complete_len=u_complete_len,
                                                                    u_input=u_input, z_input=z_input,
                                                                    m_input=m_input, turn_states=turn_states,
                                                                    degree_input=degree_input, u_input_np=u_input_np,
                                                                    m_input_np=m_input_np, u_len=u_len,
                                                                    m_len=m_len, mode='train', **kw_ret)
                sup_loss += loss.data[0]
                sup_cnt += 1
                logging.debug(
                    'loss:{} pr_loss:{} m_loss:{} c_loss:{}'.format(loss.data[0], pr_loss.data[0],
                                                                    m_loss.data[0], c_loss.data[0]))

        sup_loss /= (sup_cnt + 1e-8)
        unsup_loss /= (unsup_cnt + 1e-8)
        print('result preview...')
        res, resolution_bleu_score, user_f1, user_accuracy = self.eval(data='test')
        self.m.train()
        return sup_loss, unsup_loss, res, resolution_bleu_score, user_f1, user_accuracy
