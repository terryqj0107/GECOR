...
class ResolutionDecoder(nn.Module):
    def __init__(self, embed_size, hidden_size, vocab_size, degree_size, dropout_rate, emb, vocab):
        ...
        # self.proj_pgen2 = nn.Linear(hidden_size * 2 + embed_size, 1)
    ...
    def forward(self, history_dialogue_np, history_enc_out, z_enc_out, u_enc_out, u_input_np, m_t_input, degree_input, last_hidden, z_input_np):
        m_embed = self.emb(m_t_input)  # [1,B,E]
        z_context = self.attn_z(last_hidden, z_enc_out)  # z_context: [1,B,H]      z_enc_out : [Tz,B,H]
        u_context = self.attn_u(last_hidden, u_enc_out)  # u_context: [1,B,H]      u_enc_out : [Tu,B,H]
        gru_in = torch.cat([m_embed, u_context, z_context, degree_input.unsqueeze(0)], dim=2)
        gru_out, last_hidden = self.gru(gru_in, last_hidden)
        gen_score = self.proj(torch.cat([z_context, u_context, gru_out], 2)).squeeze(0)  # [B,V]

        sparse_z_input = Variable(self.get_sparse_selective_input(z_input_np), requires_grad=False)  # [B,Tz,V+Tz]
        z_copy_score = F.tanh(self.proj_copy2(z_enc_out.transpose(0, 1)))  # [B,Tz,H]
        z_copy_score = torch.matmul(z_copy_score, gru_out.squeeze(0).unsqueeze(2)).squeeze(2)  # [B,Tz]
        z_copy_score = z_copy_score.cpu()
        z_copy_score_max = torch.max(z_copy_score, dim=1, keepdim=True)[0]  # [B,1]
        z_copy_score = torch.exp(z_copy_score - z_copy_score_max)  # [B,Tz]
        z_copy_score = torch.log(torch.bmm(z_copy_score.unsqueeze(1), sparse_z_input)).squeeze(
            1) + z_copy_score_max     # [B,V+Tz] = torch.bmm( [B,1,Tz] , [B,Tz,V+Tz] )
        z_copy_score = cuda_(z_copy_score)  # [B,V+Tz]

        z_seq_len = z_input_np.shape[0]
        ...
        # prob_gen_1 = F.sigmoid(self.proj_pgen1(torch.cat([u_context, last_hidden, m_embed], 2)).squeeze(0))  # [B,1]
        # prob_gen_2 = F.sigmoid(self.proj_pgen2(torch.cat([u_context, last_hidden, m_embed], 2)).squeeze(0))  # [B,1]
        # prob_gen_3 = 2 - prob_gen_1 - prob_gen_2
        # prob_gen_1, prob_gen_2, prob_gen_3 = prob_gen_1 * 0.5, prob_gen_2 * 0.5, prob_gen_3 * 0.5
        # scores = F.softmax(torch.cat([gen_score * prob_gen_3, z_copy_score * prob_gen_2, u_copy_score * prob_gen_1], dim=1), dim=1)  # [B, V + V+Tz + V+Tu]

        scores = F.softmax(torch.cat([gen_score, z_copy_score, u_copy_score], dim=1), dim=1)  # [B, V + V+Tz + V+Tu]
        gen_score, z_copy_score, u_copy_score = scores[:, :cfg.vocab_size],\
                                                scores[:, cfg.vocab_size:2*cfg.vocab_size+z_seq_len],\
                                                scores[:, 2*cfg.vocab_size+z_seq_len:]    # [B,V] , [B,V+Tz] , [B,V+Tu]
        proba = gen_score + z_copy_score[:, :cfg.vocab_size] + u_copy_score[:, :cfg.vocab_size]  # [B,V] 词表范围内的词，三部分概率求和
        proba = torch.cat([proba, z_copy_score[:, cfg.vocab_size:], u_copy_score[:, cfg.vocab_size:]], 1)  # 拼接来自copy范围的概率分布
...
class ResponseDecoder(nn.Module):
    ...
    def forward(self, c_dec_outs, z_enc_out, u_enc_out, u_input_np, m_t_input, degree_input, last_hidden, z_input_np):
        sparse_z_input = Variable(self.get_sparse_selective_input(z_input_np), requires_grad=False)

        m_embed = self.emb(m_t_input)
        c_context = self.attn_c(last_hidden, c_dec_outs)
        z_context = self.attn_z(last_hidden, z_enc_out)
        u_context = self.attn_u(last_hidden, u_enc_out)
        gru_in = torch.cat([m_embed, u_context, z_context, c_context, degree_input.unsqueeze(0)], dim=2)
        # gru_in = torch.cat([m_embed, z_context, c_context, degree_input.unsqueeze(0)], dim=2)
        gru_out, last_hidden = self.gru(gru_in, last_hidden)
        gen_score = self.proj(torch.cat([z_context, u_context, c_context, gru_out], 2)).squeeze(0)
        # gen_score = self.proj(torch.cat([z_context, c_context, gru_out], 2)).squeeze(0)
...
class GECOR(nn.Module):
    ...
    def forward(self, history_dialogue_np, history_dialogue, history_dial_len, u_complete_input, u_complete_input_np, u_complete_len, u_input, u_input_np, m_input, m_input_np, z_input, u_len, m_len, turn_states,
                degree_input, mode, **kwargs):
        if mode == 'train' or mode == 'valid':
            pz_proba, pc_dec_proba, pm_dec_proba, turn_states = \
                self.forward_turn(history_dialogue_np=history_dialogue_np, history_dialogue=history_dialogue, history_dial_len=history_dial_len,
                                  u_complete_input=u_complete_input, u_complete_input_np=u_complete_input_np,
                                  u_complete_len=u_complete_len, u_input=u_input, u_len=u_len, m_input=m_input, m_len=m_len, z_input=z_input, mode='train',
                                  turn_states=turn_states, degree_input=degree_input, u_input_np=u_input_np,
                                  m_input_np=m_input_np, **kwargs)
            loss, pr_loss, m_loss, c_loss = self.supervised_loss(torch.log(pz_proba), torch.log(pm_dec_proba), torch.log(pc_dec_proba),
                                                         z_input, m_input, u_complete_input)
            return loss, pr_loss, m_loss, c_loss, turn_states

        elif mode == 'test':
            m_output_index, pz_index, turn_states, c_output_index = \
                self.forward_turn(history_dialogue_np=history_dialogue_np, history_dialogue=history_dialogue, history_dial_len=history_dial_len,
                                  u_complete_input=u_complete_input, u_complete_input_np=u_complete_input_np,
                                  u_complete_len=u_complete_len, u_input=u_input, u_len=u_len, m_input=m_input, m_len=m_len, z_input=z_input, mode='test',
                                  turn_states=turn_states, degree_input=degree_input, u_input_np=u_input_np,
                                  m_input_np=m_input_np, **kwargs)
            return m_output_index, pz_index, turn_states, c_output_index
        elif mode == 'rl':
            loss = self.forward_turn(history_dialogue_np=history_dialogue_np, history_dialogue=history_dialogue, history_dial_len=history_dial_len,
                                     u_complete_input=u_complete_input, u_complete_input_np=u_complete_input_np,
                                     u_complete_len=u_complete_len, u_input=u_input, u_len=u_len, is_train=False, mode='rl',
    ...
    def forward_turn(self, history_dialogue_np, history_dialogue, history_dial_len, u_complete_input, u_complete_input_np, u_complete_len,
                     u_input, u_len, turn_states, mode, degree_input, u_input_np, m_input_np=None,
        ...
        if mode == 'train':
            ...
            pz_proba = torch.stack(pz_proba, dim=0)  # [Tz,B,V+Tz]
            ...
            for t in range(c_length):
                ...
                proba, last_hidden, dec_out = self.r_decoder(history_dialogue_np, history_enc_out, pz_dec_outs, u_enc_out, u_input_np, c_tm1,
            ...
            for t in range(m_length):
                teacher_forcing = toss_(self.teacher_force)
                proba, last_hidden, dec_out = self.m_decoder(c_dec_outs, pz_dec_outs, u_enc_out, u_input_np, m_tm1,
                                                             degree_input, last_hidden, z_input_np)
                if teacher_forcing:
                    m_tm1 = m_input[t].view(1, -1)   # [1,B]
                else:
                    _, m_tm1 = torch.topk(proba, 1)
                    m_tm1 = m_tm1.view(1, -1)
                pm_dec_proba.append(proba)
                m_dec_outs.append(dec_out)
            pm_dec_proba = torch.stack(pm_dec_proba, dim=0)  # [Tm,B,V+Tm]

            return pz_proba, pc_dec_proba, pm_dec_proba, None
        else:
            ''' 解码bspan '''
            pz_dec_outs, bspan_index, last_hidden = self.bspan_decoder(u_enc_out, z_tm1, last_hidden, u_input_np,
                                                          pv_z_enc_out=pv_z_enc_out, prev_z_input_np=prev_z_input_np,
                                                          u_emb=u_emb, pv_z_emb=pv_z_emb)
            pz_dec_outs = torch.cat(pz_dec_outs, dim=0)  # [cfg.z_length,B,H]
            ''' 补全user utterance '''
            # last_hidden = u_enc_hidden[:-1]

            c_dec_outs, pc_dec_proba, last_hidden, c_output_index = self.c_greedy_decode(history_dialogue_np, history_enc_out, pz_dec_outs, u_enc_out, u_input_np,
                                                                   c_tm1, degree_input, last_hidden, bspan_index)

            # c_output_index = self.c_beam_search_decode(pz_dec_outs, u_enc_out, u_input_np,
            #                                                         c_tm1, degree_input, last_hidden, bspan_index)

            if mode == 'test':
                if not self.beam_search:
                    m_output_index = self.greedy_decode(c_dec_outs, pz_dec_outs, u_enc_out, m_tm1, u_input_np, last_hidden,
                                                        degree_input, bspan_index)

                else:
                    m_output_index = self.beam_search_decode(c_dec_outs, pz_dec_outs, u_enc_out, m_tm1, u_input_np, last_hidden,
                                                             degree_input, bspan_index)

                return m_output_index, bspan_index, None, c_output_index
            elif mode == 'rl':
                return self.sampling_decode(c_dec_outs, pz_dec_outs, u_enc_out, m_tm1, u_input_np, last_hidden,
                                            degree_input, bspan_index)
    ...
    def c_decoder(self, u_complete_input, pz_dec_outs, u_enc_out, u_input_np, c_tm1, degree_input, last_hidden, bspan_index):
        pc_dec_proba, c_dec_outs = [], []
        # z_input_np = z_input.cpu().data.numpy()
        z_input_np = pad_sequences(bspan_index).transpose((1, 0))
        c_length = u_complete_input.size(0)  # Tc
        for t in range(c_length):
            teacher_forcing = toss_(self.teacher_force)
            proba, last_hidden, dec_out = self.r_decoder(pz_dec_outs, u_enc_out, u_input_np, c_tm1,
                                                         degree_input, last_hidden, z_input_np)
            if teacher_forcing:
                c_tm1 = u_complete_input[t].view(1, -1)  # [1,B]
            else:
            ...
            c_dec_outs.append(dec_out)
        c_dec_outs = torch.cat(c_dec_outs, dim=0)  # [Tc,B,H]
        pc_dec_proba = torch.stack(pc_dec_proba, dim=0)  # [Tc,B,V]
        return c_dec_outs, pc_dec_proba, last_hidden

    ''' 新增 '''
    def c_greedy_decode(self, history_dialogue_np, history_enc_out, pz_dec_outs, u_enc_out, u_input_np, c_tm1, degree_input, last_hidden, bspan_index):
        pc_dec_proba, c_dec_outs = [], []
        decoded = []
        z_input_np = pad_sequences(bspan_index).transpose((1, 0))
    ...
    def greedy_decode(self, c_dec_outs, pz_dec_outs, u_enc_out, m_tm1, u_input_np, last_hidden, degree_input, bspan_index):
        decoded = []
        bspan_index_np = pad_sequences(bspan_index).transpose((1, 0))
        for t in range(self.max_ts):
            proba, last_hidden, _ = self.m_decoder(c_dec_outs, pz_dec_outs, u_enc_out, u_input_np, m_tm1,
    ...
    ''' 新增 '''
    def c_beam_search_decode_single(self, pz_dec_outs, u_enc_out, c_tm1, u_input_np, last_hidden, degree_input,
                                  bspan_index):
        ...
        for t in range(self.max_ts):
            ...
            while k < len(states) and k < self.beam_size - dead_k:
                ...
                proba, last_hidden, dec_out = self.r_decoder(pz_dec_outs, u_enc_out, u_input_np, m_tm1, degree_input,
        ...
        c_dec_outs = torch.cat(c_dec_outs, dim=0)
        ...
        return generated, c_dec_outs, last_hidden

    ''' 新增 '''
    def c_beam_search_decode(self, pz_dec_outs, u_enc_out, u_input_np, c_tm1, degree_input, last_hidden, bspan_index):
        vars = torch.split(pz_dec_outs, 1, dim=1), torch.split(u_enc_out, 1, dim=1),\
               torch.split(c_tm1, 1, dim=1), torch.split(last_hidden, 1, dim=1), torch.split(degree_input, 1, dim=0)
        decoded = []
        for i, (pz_dec_out_s, u_enc_out_s, c_tm1_s, last_hidden_s, degree_input_s) in enumerate(zip(*vars)):
            decoded_s, c_dec_outs, last_hidden = self.c_beam_search_decode_single(pz_dec_out_s, u_enc_out_s, c_tm1_s,
    ...
    def beam_search_decode_single(self, c_dec_outs, pz_dec_outs, u_enc_out, m_tm1, u_input_np, last_hidden, degree_input,
                                  bspan_index):
        eos_token_id = self.vocab.encode(cfg.eos_m_token)
        batch_size = pz_dec_outs.size(1)
        if batch_size != 1:
        ...
        for t in range(self.max_ts):
            ...
            while k < len(states) and k < self.beam_size - dead_k:
                state = states[k]
                last_hidden, m_tm1 = state.last_hidden, state.decoded[-1]
                proba, last_hidden, _ = self.m_decoder(c_dec_outs, pz_dec_outs, u_enc_out, u_input_np, m_tm1, degree_input,
                                                       last_hidden, bspan_index_np)
    ...
    def beam_search_decode(self, c_dec_outs, pz_dec_outs, u_enc_out, m_tm1, u_input_np, last_hidden, degree_input, bspan_index):
        vars = torch.split(c_dec_outs, 1, dim=1), torch.split(pz_dec_outs, 1, dim=1), torch.split(u_enc_out, 1, dim=1),\
               torch.split(m_tm1, 1, dim=1), torch.split(last_hidden, 1, dim=1), torch.split(degree_input, 1, dim=0)
        decoded = []
        for i, (c_dec_out_s, pz_dec_out_s, u_enc_out_s, m_tm1_s, last_hidden_s, degree_input_s) in enumerate(zip(*vars)):
            decoded_s = self.beam_search_decode_single(c_dec_out_s, pz_dec_out_s, u_enc_out_s, m_tm1_s,
                                                       u_input_np[:, i].reshape((-1, 1)),
                                                       last_hidden_s, degree_input_s, bspan_index[i])
            decoded.append(decoded_s)
        return [list(_.view(-1)) for _ in decoded]

    def supervised_loss(self, pz_proba, pm_dec_proba, pc_dec_proba, z_input, m_input, u_complete_input):
        # pz_proba, pm_dec_proba, pc_dec_proba = pz_proba[:, :, :cfg.vocab_size].contiguous(),\
        #                                        pm_dec_proba[:, :, :cfg.vocab_size].contiguous(), \
        #                                        pc_dec_proba[:, :, :cfg.vocab_size].contiguous()
        pz_proba, pm_dec_proba, pc_dec_proba = pz_proba.contiguous(), \
                                               pm_dec_proba.contiguous(), \
                                               pc_dec_proba.contiguous()
        ...
        loss = pr_loss + m_loss + c_loss
        return loss, pr_loss, m_loss, c_loss
    ...
    def sampling_decode(self, c_dec_outs,  pz_dec_outs, u_enc_out, m_tm1, u_input_np, last_hidden, degree_input, bspan_index):
        vars = torch.split(c_dec_outs, 1, dim=1), torch.split(pz_dec_outs, 1, dim=1), torch.split(u_enc_out, 1, dim=1),\
               torch.split(m_tm1, 1, dim=1), torch.split(last_hidden, 1, dim=1), torch.split(degree_input, 1, dim=0)
        batch_loss = []
        sample_num = 1
        for i, (c_dec_out_s, pz_dec_out_s, u_enc_out_s, m_tm1_s, last_hidden_s, degree_input_s) in enumerate(zip(*vars)):
            ...
            for j in range(sample_num):
                loss = self.sampling_decode_single(c_dec_out_s, pz_dec_out_s, u_enc_out_s, m_tm1_s, u_input_np[:, i].reshape((-1, 1)),
                                                   last_hidden_s, degree_input_s, bspan_index[i])
    ...
    def sampling_decode_single(self, c_dec_outs, pz_dec_outs, u_enc_out, m_tm1, u_input_np, last_hidden, degree_input, bspan_index):
        decoded = []
        reward_sum = 0
        log_probs = []
        rewards = []
