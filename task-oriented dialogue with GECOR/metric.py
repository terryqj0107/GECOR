...
class CamRestEvaluator(GenericEvaluator):
    ...
    def run_metrics(self):
        ...
        for i, row in enumerate(data):
            ...
            # data[i]['user_complete'] = self.clean(data[i]['user_complete'])
            # data[i]['generated_user'] = self.clean(data[i]['generated_user'])
        ...
        return success_f1[0], resolution_bleu_score, user_f1, user_accuracy
    ...
    def success_f1_metric(self, data, sub='successf1'):
        ...
        for dial_id in dials:
            ...
            for turn_num, turn in enumerate(dial):
                ...
                if not operator.eq(user_token, user_complete_token):  # 输入句子不完整
                    if operator.eq(gen_user_token, user_complete_token):  # 补全成功
                        # print('user_token:', user_token)
                        # print('gen_user_token:', gen_user_token)
                        # print('gen_response:', gen_response_token)
                        # print('response:', response_token)
                        # print('***************************************')
                        pass
                    elif operator.eq(gen_user_token, user_token):  # 未补全
                        # print('user_token:', user_token)
                        # print('gen_user_token:', gen_user_token)
                        # print('user_complete_token:', user_complete_token)
                        # print('gen_response:', gen_response_token)
                        # print('response:', response_token)
                        # print('***************************************')
                        pass
