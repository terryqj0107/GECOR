...
class _ReaderBase:
    ...
    def wrap_result(self, turn_batch, gen_c, gen_m, gen_z, eos_syntax=None, prev_z=None):
        """
        ...
        if eos_syntax is None:
            eos_syntax = {'response': 'EOS_M', 'user': 'EOS_U', 'user_complete': 'EOS_U', 'bspan': 'EOS_Z2'}
        ...
        for i in range(batch_size):
            ...
            if gen_c:
                entry['generated_user'] = self.vocab.sentence_decode(gen_c[i], eos='EOS_U')
            else:
                entry['generated_user'] = ''
            if gen_m:
            ...
            # print("generated_user: ", entry['generated_user'])
        write_header = False
        ...
        field = ['dial_id', 'turn_num', 'generated_user', 'user_complete', 'user', 'generated_bspan', 'bspan',
...
class CamRest676Reader(_ReaderBase):
    ...
    def _get_tokenized_data(self, raw_data, db_data, construct_vocab):
        ...
        for dial_id, dial in enumerate(raw_data):
            ...
            for turn in dial['dial']:
                ...
                # user_complete = word_tokenize(turn['usr']['transcript_complete']) + ['EOS_U']
                user_complete = word_tokenize(self._replace_entity_in_user(turn['usr']['transcript_complete'], vk_map, constraint)) + ['EOS_U']
                # print(user_complete)
                ...
                if count % 2 == 1:
                    ...
                    if turn['usr']['transcript_with_ellipsis'] != '':
                        user_incomplete = word_tokenize(
                            self._replace_entity_in_user(turn['usr']['transcript_with_ellipsis'], vk_map,
                                                         constraint)) + ['EOS_U']
                        ellipsis += 1
                    else:
                        if turn['usr']['transcript_with_coreference'] != '':
                            user_incomplete = word_tokenize(
                                self._replace_entity_in_user(turn['usr']['transcript_with_coreference'], vk_map,
                                                             constraint)) + ['EOS_U']
                            coreference += 1
                        else:
                            user_incomplete = word_tokenize(
                                self._replace_entity_in_user(turn['usr']['transcript_complete'], vk_map,
                                                             constraint)) + ['EOS_U']
                else:
                    count += 1
                    if turn['usr']['transcript_with_coreference'] != '':
                ...
                # if turn['usr']['transcript_with_ellipsis'] != '':
                #     user_incomplete = word_tokenize(
                #         self._replace_entity_in_user(turn['usr']['transcript_with_ellipsis'], vk_map, constraint)) + ['EOS_U']
                # else:
                #     user_incomplete = word_tokenize(
                #         self._replace_entity_in_user(turn['usr']['transcript_complete'], vk_map, constraint)) + ['EOS_U']

                ''' 指代 '''
                # if turn['usr']['transcript_with_coreference'] != '':
                #     user_incomplete = word_tokenize(
                #         self._replace_entity_in_user(turn['usr']['transcript_with_coreference'], vk_map, constraint)) + ['EOS_U']
                # else:
                #     user_incomplete = word_tokenize(
                #         self._replace_entity_in_user(turn['usr']['transcript_complete'], vk_map, constraint)) + ['EOS_U']
                ...
                if construct_vocab:
                    for word in user_complete + response + constraint + requested:
                        self.vocab.add_item(word)
            tokenized_data.append(tokenized_dial)
        print('ellipsis:{}  coreference:{}'.format(ellipsis, coreference))
    ...
    ''' 新增，替换user utterance中的命名实体 '''
    def _replace_entity_in_user(self, response, vk_map, constraint):
        response = re.sub('[cC][., ]*[bB][., ]*\d[., ]*\d[., ]*\w[., ]*\w', 'postcode_SLOT', response)
        response = re.sub('\d{5}\s?\d{6}', 'phone_SLOT', response)
        constraint_str = ' '.join(constraint)
        for v, k in sorted(vk_map.items(), key=lambda x: -len(x[0])):
    ...
    def _get_encoded_data(self, tokenized_data):
        ...
        for dial in tokenized_data:
            ...
            history_dialogue = []
            for turn in dial:
                user_complete = self.vocab.sentence_encode(turn['user_complete'])
                # user_incomplete = self.vocab.sentence_encode(turn['user_incomplete'])
                ...
                encoded_dial.append({
                    ...
                    # 'user': prev_response + user,   # 上轮系统回复 + 本轮用户语句
                    ...
                    'user_complete': user_complete,
                    'u_complete_len': len(user_complete),
                    # 'user_incomplete': user_incomplete,
                    # 'u_incomplete_len': len(user_incomplete),
                    'history_dialogue': history_dialogue + user,
                    'history_dial_len': len(history_dialogue + user)
                })
                # modified
                prev_response = response
                history_dialogue = history_dialogue + user + response  # 历史对话累加
